# download html and convert it to a text file
...
# get html site from a given address, convert it to text 
# and store it to filename
def html2textfile(address, filename):
	
	print "\nTrying to download %s ......" % (filename)
	...
	try:
		...
		print "X Could not download %s" %(address)
		return
	
	try:
		file_out= open(filename,"w")
		html2text_file(f.read(), file_out.write)
		file_out.close()
		print "--> Wrote data to %s" % (filename)
	except:
		print "X Can't read %s" %(address)
		return
...
#		data = sys.stdin.read()
#	html2text_file(data)
	html2textfile(sys.argv[1], "a.out")
