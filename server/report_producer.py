...
'''
    ...
    Intended for sun solaris server.
'''

db = create_engine("mysql://username:password@localhost:3306/spree_dev?unix_socket=/home/milena/mysql/mysqld.sock")
#db = create_engine("mysql://username:password@localhost:3306/spree_dev")

metadata = BoundMetaData(db)


query_log = Table('query_log', metadata,
    Column('qlog_id', Integer),
    Column('query_id', Integer),
    Column('user_name', String(16)),
    Column('user_id', Integer),
    Column('created', DateTime),
    Column('status', String(20)),
    Column('isDirect',Integer(1))
)

query = Table('query', metadata,
    Column('query_id', Integer),
    Column('isDirect', Integer(1)),
    Column('profile_subtree',mysql.MSText),
    Column('profile_calculated_subtree',mysql.MSText),
    Column('text', TEXT)
)

r=select([query_log.c.query_id, query_log.c.user_id, query_log.c.user_name, query_log.c.status, query_log.c.created]).execute()
#r=select(query_log).execute()
rows=r.fetchall()
...
user_log = Table('user_log', metadata,
    Column('ulog_id', Integer),
    Column('user_name', String(16)),
    Column('user_id', Integer),
    Column('login_time', DateTime),
    Column('logout_time', DateTime),
    Column('last_polling', DateTime),
)
...
chat_log = Table('chat_log', metadata,
    Column('clog_id', Integer),
    Column('query_id', Integer),
    Column('user_id', Integer),
    Column('user_name', String(16)),
    Column('expert_id', Integer),
    Column('expert_name', String(16)),
    Column('exp_accepted', DateTime),
    Column('user_joined', DateTime),
    Column('who_ended', String(16)),
    Column('chat_ended', DateTime),
)
